...
<body>
    <div class="email-container">
        ...
        <div class="email-body">
            <p>To <strong>HR Department of PT. Asian Bay Development</strong>,</p>
            <p>
                Manager named <strong>{{ $submission->submitter->employee_name }}</strong> has submitted a new position request.
            </p>
            <h3>Position Request Details</h3>
            <ul>
                <li><strong>Position Request:</strong> {{ $submission->positionRelation->name ?? 'N/A' }}</li>
                <li><strong>Location:</strong> {{ $submission->store->name ?? 'N/A' }}</li>
            ...
            <p style="margin:20px 0;">
                <a href="https://hr.unclejo.xyz" target="_blank" class="btn">Open HR Dashboard</a>
            </p>
        ...
        <div class="email-body">
            ...
            <p>
                Manajer bernama <strong>{{ $submission->submitter->employee_name }}</strong> telah mengajukan permintaan posisi baru.
            </p>

            <h3>Detail Permintaan Posisi</h3>
            <ul>
                <li><strong>Posisi yang Diajukan:</strong> {{ $submission->positionRelation->name ?? 'Tidak ada' }}</li>
                <li><strong>Lokasi:</strong> {{ $submission->store->name ?? 'Tidak ada' }}</li>
                <li><strong>Ringkasan Peran:</strong> {!! $submission->role_summary ?? '<em>Kosong</em>' !!}</li>
                <li><strong>Tanggung Jawab Utama:</strong> {!! $submission->key_respon ?? '<em>Kosong</em>' !!}</li>
                <li><strong>Kualifikasi:</strong> {!! $submission->qualifications ?? '<em>Kosong</em>' !!}</li>
