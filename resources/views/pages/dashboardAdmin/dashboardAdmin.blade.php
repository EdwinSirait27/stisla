...
@push('styles')
    ...
    <style>
        ...
        .act-btn:hover { background: #f8fafc; color: #1e293b; }
        ...
        div.dataTables_wrapper div.dataTables_paginate { padding-top: .3rem; }
        div.dataTables_wrapper div.dataTables_paginate .paginate_button {
            font-size: .75rem;
            border-radius: .375rem !important;
            padding: .2rem .5rem;
        }
        .dataTables_wrapper { padding: .75rem 1.25rem 1rem; }
...
jQuery(document).ready(function ($) {
    ...
    var table = $('#users-table').DataTable({
        ...
        columns: [
            ...
            { data: 'grading_name',   name: 'Employee.grading.grading_name', className: 'text-center' },
            { data: 'store_name',     name: 'Employee.store.name',        className: 'text-center' },
            ...
            },
            {
                ...
                render: d => d ? `<code style="font-size:.72rem;color:#475569">${d}</code>` : '<span style="color:#cbd5e1">—</span>'
            },
            {
            ...
            },
            {
                ...
                render: d => d ? `<span class="role-badge"><i class="fas fa-shield-halved"></i>${d}</span>` : '-'
            },
            {
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.success) {
                        Swal.fire({ icon: 'success', title: 'Updated!', text: res.message, confirmButtonColor: '#6777ef' });
                        table.ajax.reload(null, false);
                    } else {
                ...
                error: function () {
                    Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to update users. Please try again.' });
                }
            });
        });
    });
    ...
    /* ── Session flash ── */
    @if (session('success'))
    Swal.fire({ icon: 'success', title: 'Success', text: '{{ session('success') }}', confirmButtonColor: '#6777ef', timer: 3000, timerProgressBar: true });
    @endif
