@extends('layouts.app')
@section('title', 'Update Public Holiday')
@push('styles')
    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>Update Public Holiday {{ $pubholi->remark }}</h1>
                <div class="section-header-breadcrumb">
                    {{-- <div class="breadcrumb-item active"><a href="{{ route('dashboard') }}">Dashboard</a></div> --}}
                    <div class="breadcrumb-item"><a href="{{ route('pages.Pubholi') }}">Public Holiday</a></div>
                    <div class="breadcrumb-item">Update Public Holiday {{ $pubholi->remark }}</div>
                </div>
            </div>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header pb-0 px-3">
                                    <h6 class="mb-0">{{ __('Update Public Holiday') }} {{ $pubholi->remark }}</h6>
                                </div>
                                <div class="card-body pt-4 p-3">
                                    ...
                                    <form id="departments-edit" action="{{ route('Pubholi.update', $hashedId) }}"
                                        method="POST">
                                        @csrf
                                        @method('PUT')
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="date" class="form-control-label">
                                                        <i class="fas fa-id-card"></i> {{ __('Date') }}
                                                    </label>
                                                    <div>
                                                        <input type="date" name="date"
                                                            value="{{ $pubholi->date ? \Carbon\Carbon::parse($pubholi->date)->format('Y-m-d') : '' }}"
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                     <label for="remark" class="form-control-label">
                                                        <i class="fas fa-user"></i> {{ __('Remark') }}
                                                    </label>
                                                    <div>
                                                        <input type="text"
                                                            class="form-control @error('remark') is-invalid @enderror"
                                                            id="remark" name="remark"
                                                            value="{{ old('remark', $pubholi->remark) }}"
                                                            placeholder="Insert remark" required>
                                                        @error('remark')
                                                            <span class="invalid-feedback" role="alert">
                                                                <strong>{{ $message }}</strong>
                                                            </span>
                                                    ...
                                                    <div>
                                                        <select name="type"
                                                            ...
                                                            @foreach ($status_type as $value)
                                                                <option value="{{ $value }}"
                                                                    {{ old('type', $pubholi->type ?? '') == $value ? 'selected' : '' }}>
                                                                    {{ $value }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                        ...
                                        <div class="alert alert-secondary mt-4" role="alert">
                                            <span class="text-dark">
                                                ...
                                                - If a Remark  is already registered, you cannot register it
                                                again.<br>
