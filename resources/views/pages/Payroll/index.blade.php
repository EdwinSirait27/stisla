...
@push('styles')
    ...
    <style>
        ...
        /* Period info bar */
        .period-bar { background: #eff6ff; border: 1px solid #bfdbfe; border-radius: .625rem; padding: .75rem 1.25rem; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 1rem; flex-wrap: wrap; }
        .period-bar-label { font-size: .78rem; font-weight: 600; color: #1e40af; }
        .period-bar-range { font-size: .78rem; color: #3b82f6; }
        .period-bar-status { display: inline-flex; align-items: center; padding: .18rem .6rem; border-radius: 20px; font-size: .7rem; font-weight: 700; }
        ...
        .dt-toolbar select { height: 32px; font-size: .775rem; border: 1px solid #e2e8f0; border-radius: .4rem; padding: 0 .6rem; }

        /* Bulk action bar */
        .bulk-bar { display: none; padding: .6rem 1.25rem; background: #eff6ff; border-bottom: 1px solid #bfdbfe; align-items: center; gap: .75rem; }
        .bulk-bar.show { display: flex; }
        .bulk-bar-count { font-size: .8rem; font-weight: 600; color: #1e40af; }
        .bulk-bar .btn { height: 30px; font-size: .775rem; }

        /* Table */
        #payroll-table { width: 100% !important; font-size: .8rem; }
        #payroll-table thead th { background: #f8fafc; color: #64748b; font-size: .68rem; font-weight: 700; letter-spacing: .5px; padding: .7rem .9rem; border: none; border-bottom: 1px solid #f1f5f9; white-space: nowrap; }
        #payroll-table tbody td { padding: .75rem .9rem; vertical-align: middle; border: none; border-bottom: 1px solid #f8fafc; color: #334155; }
        ...
        .emp-avatar { width: 30px; height: 30px; border-radius: 50%; font-size: .65rem; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .emp-avatar-name { font-weight: 600; font-size: .8rem; line-height: 1.2; }
        .emp-avatar-id { font-size: .7rem; color: #94a3b8; }
        ...
        .act-btn.act-success { border-color: #bbf7d0; background: #f0fdf4; color: #16a34a; }
        ...
        <div class="section-header">
            <div>
                <div style="font-size:.72rem;color:#94a3b8;margin-bottom:3px">
                    Dashboard /
                    <a href="{{ route('payrollperiod.index') }}" style="color:#64748b;text-decoration:none">Payroll Periods</a> /
            ...
            <div class="page-actions">
                @if($period->isOpen())
                    <a href="{{ route('payroll.generate', $period->id) }}"
...
@push('scripts')
    ...
    <script>
        $(function () {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: (data, type, row) => {
                            ...
                            const c = colors[row.status_employee] ?? { bg: '#f1f5f9', color: '#475569' };
                    ...
                    { data: 'status_employee_badge', orderable: false },
                    {
                        data: null,
                        render: (d, t, row) =>
                    ...
                    { data: 'gross_salary_fmt', className: 'num' },
                    { data: 'total_deduction_fmt', className: 'num', render: d => `<span style="color:#dc2626">${d}</span>` },
                    { data: 'net_salary_fmt', className: 'num', render: d => `<strong>${d}</strong>` },
                    { data: 'prorate_info', orderable: false, defaultContent: '-' },
            ...
            $('#btn-reset').on('click', () => {
                $('#filter-status, #filter-status-employee, #filter-store').val('');
                table.ajax.reload();
            });
            ...
            $(document).on('click', '.btn-approve', function () {
                ...
                Swal.fire({
                    ...
                    .fail(err => {
                        Swal.fire({ icon: 'error', title: 'Gagal!', text: err.responseJSON?.error ?? 'Terjadi kesalahan.' });
                    });
                });
            });
            ...
            $('#btn-do-bulk-approve').on('click', function () {
                ...
                Swal.fire({
                    ...
                    .done(res => {
                        Swal.fire({ icon: 'success', title: 'Bulk Approved!', timer: 2000, showConfirmButton: false });
                        $('.row-check, #check-all').prop('checked', false);
                        updateBulkBar();
                        table.ajax.reload();
                    })
