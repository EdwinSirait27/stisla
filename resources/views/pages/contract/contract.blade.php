    ...
    <div class="main-content">
        <section class="section">
            ...
            <div class="section-header">
                <div>
                    <div style="font-size:.72rem;color:#94a3b8;margin-bottom:3px">
                        Dashboard / <span style="color:#64748b">Contracts</span>
                    </div>
                    <h1>Contracts</h1>
                </div>
                <div class="page-actions">
                    <a href="{{ route('createcontract') }}" class="btn btn-primary">
                        <i class="fas fa-plus"></i> Create Contracts
                    </a>
                    </div>
            </div>
            ...
            <div class="section-body">
                ...
                <div class="stats-row">
                    <div class="stat-card">
                        <div class="stat-card-label">Total Contracts</div>
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-card-value green" id="stat-active">{{ $countactivecontracts }}</div>
                        <div class="stat-card-sub">
                            <span class="stat-dot" style="background:#16a34a"></span> Active Contracts
                        </div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-card-label">Expired Contracts</div>
                        <div class="stat-card-value amber" id="stat-leave">{{ $countexpiredcontracts }}</div>
                        <div class="stat-card-sub">
                            <span class="stat-dot" style="background:#d97706"></span> Expired Contracts
                        </div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-card-label">Terminated Contracts</div>
                        <div class="stat-card-value red" id="stat-resign">{{ $countterminatedcontracts }}</div>
                        <div class="stat-card-sub">
                            <span class="stat-dot" style="background:#dc2626"></span> Terminated Contracts
                        </div>
                    </div>
                </div>
                ...
                <div class="emp-card">
                    <div class="emp-card-header">
                        ...
                        <span class="emp-card-header-title">List Contracts</span>
...
">
                        ...
                        {{-- <select id="filter-contract-status" class="select2 form-select form-select-sm"
                            style="height:32px;font-size:.775rem;border:1px solid #e2e8f0;border-radius:.4rem;min-width:125px;">
                        ...
                        <select id="filter-group" class="select2 form-select form-select-sm"
                         ...
                         <select id="filter-contract-type" class="select2 form-select form-select-sm"
                            style="height:32px;font-size:.775rem;border:1px solid #e2e8f0;border-radius:.4rem;min-width:125px;">
                        ...
                        <select id="filter-contract-status" class="select2 form-select form-select-sm"
                          ...
                          <select id="filter-employee-status" class="select2 form-select form-select-sm"
                            style="height:32px;font-size:.775rem;border:1px solid #e2e8f0;border-radius:.4rem;min-width:125px;">
                ...
                <div class="emp-card">
                    <div class="emp-card-header">
                        ...
                        <span class="emp-card-header-title">Contract Activity history</span>
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        $(function() {
            var empTable = $('#employees-table').DataTable({
                ...
                ajax: {
                    ...
                    data: function(d) {
                        ...
                        d.filter_contract_type = $('#filter-contract-type').val();
                        d.filter_contract_status = $('#filter-contract-status').val();
                        d.filter_employee_status = $('#filter-employee-status').val();
                        // d.filter_status_employee = $('#filter-status-employee').val();
                    },
                    error: function() {
                        Swal.fire({
                            icon: 'error',
                ...
                columns: [
                    ...
                    },
                    {
                     ...
                     { data: 'contract_type', className: 'text-center',name: 'contract_type', render: d => statusBadge(d, CONTRACT_TYPE) },
                     { data: 'contract_status', className: 'text-center',name: 'contract_status', render: d => statusBadge(d, CONTRACT_STATUS) },
                    
                    {
                        data: 'employee_status',
                        name: 'employee_status',
                        className: 'text-center',
                        render: d => statusBadge(d, EMPLOYEE_STATUS_BADGE)
                    },
                     { data: 'created_at', className: 'text-center',name: 'created_at'},
            ...
            $('#filter-company, #filter-department,#filter-store, #filter-grading, #filter-group, #filter-contract-type, #filter-contract-status, #filter-employee-status')
                .on('change', function() {
            ...
            $('#btn-reset-filter').on('click', function() {
                ...
                $('#filter-contract-type').val('').trigger('change');
                $('#filter-contract-status').val('').trigger('change');
                $('#filter-employee-status').val('').trigger('change');
                empTable.ajax.reload();
            });
            ...
            function updateExportLinks() {
                const params = new URLSearchParams({
                    ...
                    filter_contract_type: $('#filter-contract-type').val(),
                    filter_contract_status: $('#filter-contract-status').val(),
                    filter_employee_status: $('#filter-employee-status').val(),
                });
            ...
            $('#activityTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('datacontracts.datacontracts') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
