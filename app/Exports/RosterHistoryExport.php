...
class RosterHistoryExport implements WithMultipleSheets
{
    public function __construct(
        ...
        protected ?string $myStoreId,
         protected array $myStoreIds = [],    // ← tambah
        protected array $myDeptIds = [],     // ← tambah
        protected array $bawahanIds = [],    // ← tambah
    ...
    public function sheets(): array
    {
        return [
            new RosterHistorySheet(
                ...
                $this->canManageAll,
                $this->myStoreId,
                $this->myStoreIds,    // ← tambah
                $this->myDeptIds,     // ← tambah
                $this->bawahanIds,    // ← tambah
                $this->storeName,
...
class RosterHistorySheet implements WithEvents, WithTitle, ShouldAutoSize
{
    ...
    // ← SPV: pattern sama dengan controller
    $q->where(function ($q2) {
        // Karyawan reguler: store + department SPV
        $q2->where(function ($q3) {
            if ($this->storeId) {
                $q3->whereHas('store', fn($sq) =>
                    $sq->where('stores_tables.id', $this->storeId)
                );
            } elseif (!empty($this->myStoreIds)) {
                $q3->whereHas('store', fn($sq) =>
                    $sq->whereIn('stores_tables.id', $this->myStoreIds)
                );
            }

            if (!empty($this->myDeptIds)) {
        ...
        // Bawahan langsung — bypass store/department/company
        if (!empty($this->bawahanIds)) {
            $q2->orWhereIn('id', $this->bawahanIds);
        }
    });
}
                })
                ...
                // Sort berdasarkan position name A-Z, lalu employee name A-Z
                $grouped = $query->groupBy('employee_id')
                    ->sortBy(function($items) {
                        $emp          = $items->first()->employee;
                        $positionName = $emp?->position?->first()?->name ?? 'ZZZZ';
                        $empName      = $emp?->employee_name ?? 'ZZZZ';
                        return $positionName . '___' . $empName;
                    });
                $rowNum  = 5;
