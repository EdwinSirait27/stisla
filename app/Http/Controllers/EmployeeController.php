...
//         if (
//             $user->hasPermissionTo('ViewEmployee')
//             && !$user->hasPermissionTo('ManageEmployee')
...
//     && !$user->hasPermissionTo('ManageEmployee')
// ) {
//     // ← Stores kepunyaan SPVManager saja, orderby name
//     $stores = $user->employee->store()
//         ->orderBy('stores_tables.name')
//         ->get();

//     $departments = $user->employee->department()->get();
...
public function index()
{
    ...
    // Data umum untuk semua permission
    $countactives     = Employee::where('status', 'Active')->count();
    $countpendings    = Employee::where('status', 'Pending')->count();
    $companies        = Company::pluck('name', 'id');
    $employeestatuses = Employee::getStatusEmployeeOptions();
    ...
    if (
        ...
        $stores      = $user->employee->store()->orderBy('stores_tables.name')->get();
        $departments = $user->employee->department()->get();
...
if ($canSpvManager) {
    ...
    public function edit($hashedId)
    {
        ...
        $marriage = ['Yes', 'No'];
        $bloodtypes = Employee::getBloodTypeOptions();
        $attendancetypes = Employee::getAttendanceTypeOptions();
    ...
    public function create()
    {
        ...
        return view('pages.Employee.create', compact('attendancetypes','atasans', 'employeestatuses', 'gradings', 'employees', 'bloodtypes', 'companys', 'stores', 'banks', 'status_marriage', 'positions', 'departments', 'status_employee', 'status_child', 'status_gender', 'status_religion', 'status_last_education', 'status'));
    }
    public function store(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'attendance_type' => ['nullable', 'string', 'max:255', new NoXSSInput()],
            ...
            'kk_number' => ['nullable', 'max:20', new NoXSSInput()],
    ...
    */
        ...
        try {
            ...
            $employees = Employee::create([
                ...
                'attendance_type' => $validatedData['attendance_type'] ?? null,
